...
These different prior probabilities can be updated using the INCEPTION to create posterior distributions that are summarized in Table 2. Basically, it is observed that in all cases the point estimate for the enhanced eCPR survival remains fairly constant but the associated uncertainty is reduced with the additional data as reflected by the narrower 95% CrI. The posterior probability of improved eCPR survival goes from 80% with a skeptical prior to 85% with the enthusiastic prior.
...
gt(table_tbl_2) %>%
  tab_header(
    title = "Table 2 Probabilities of improved survival with eCPR with various priors"
  ) %>%
  tab_source_note(
