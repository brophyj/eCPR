...
[@mcelreath2020]. Bayesian analyses permit not only calculated the posterior probability of
any additional survival with eCPR (OR \>1.00), but also of clinically meaningful benefits. While there is no universal standard for a clinically meaningful benefit, a survival OR \>1.10 may be an acceptable threshold for many. Bayesian analyses also allows calculation of the probability between any two points. For example, rather than simply comparing if the survival of one treatment is better, one can calculate a range of practical equivalence (ROPE). While different ranges may be proposed as equivalent, +/- 10% seems a reasonable starting point.             
...
ITT data from the INCEPTION trial[@CPR2023a] and two other pertinent trials[@RN6751;@RN6759] that also randomized CPR to eCPR are shown in Table 1. Performing a Bayesian analysis on the INCEPTION trial, using the default vague priors, produces an odds ratio (OR) 1.32 (95% Credible Interval (CrI) 0.54 - 3.22). The closeness of this result to the original analysis (OR, 1.4; 95% CI 0.5 - 3.5) confirms minimal impact of the vague prior and shows this Bayesian analysis is completely dominated by the observed INCEPTION data.  
